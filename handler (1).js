...
module.exports.defaultHandler = (event, context, callback) => {
  ...
  if(Op == 98){
    getSubscriptionDetails(event).then(()=>{
      callback(null, successfullResponse)
    }).catch((err)=>{
      console.log(err);
      callback(null, JSON.stringify(err));
  ...
  if(Op == 5){
    ...
    event.body = JSON.stringify({bstate:'1',auth:data.auth});
    return send(event, connectionId);
  }
  if(Op == 6){
    if(data.type == 0){
      getBoardsConnected(event).then(()=>{
        callback(null, successfullResponse)
      }).catch((err)=>{
        console.log(err);
        callback(null, JSON.stringify(err));
...
const getSubscriptionDetails = async (event) =>{
  ...
  if(connectionData.Count > 0){
    return connectionData.Items.map((data) => {
      ...
      //console.log(data);
      event.data = JSON.stringify({Op: 98, isSubscribed: data.isSubscribed, data: data});    
      event.body = JSON.stringify({Op: 98, isSubscribed: data.isSubscribed, data: data});     
      return send(event, connectionId);
    });
  }
  else{
...
const getBoardsConnected = async (event) =>{
  ...
  if(connectionData)
    event.data = JSON.stringify({bstate:1});
  else
    event.data = JSON.stringify({bstate:0});
  //console.log(connectionData);
  return send(event, connectionId);
};
  ...
  return connectionData.Items.map((connectionId) => {
    ...
    if(type == 0){
      if(connectionId.bssid == tbssid){
        ...
        //event.body.type = 1; //changed hardware side
        if(connectionId.connectionId != tconnectionId)
          return send(event, connectionId.connectionId);
      }   
    }
    if(type == 1){
      if(connectionId.bssid == usercode){ // needs to change to user code.... 
        //console.log(connectionId);
        if(connectionId.connectionId != tconnectionId)
          return send(event, connectionId.connectionId);
      }
